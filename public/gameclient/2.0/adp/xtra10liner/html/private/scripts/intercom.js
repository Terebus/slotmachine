...
var Intercom = (function() {
	...
	var hasLocalStorage = (function(){
		...
		try{
			...
			console.warn('intercom cant be used if localstorage api is deactivated, did you turn off your cookies?');
			return false;
		}
    ...
    /**
	 * executes a function if the last execution happend "delay" milliseconds before
     * @param delay
     * @param fn
     * @returns {Function}
     */
	...
	*/
	...
	Intercom.prototype._cleanup_emit = util.throttle(100, function() {
		...
		this._transaction(function() {
			var now = intercomNow();//(new Date()).getTime();
		...
		if (this._localStorageChanged(event, INDEX_EMIT)) {
			this._transaction(function() {
				...
				for (var i = 0; i < messages.length; i++) {
					...
					//trigger the message to all listeners
	...
	Intercom.prototype._emit = function(name, message, id) {
		...
		this._transaction(function() {
			...
			var delimiter = (data === '[]') ? '' : ',';
			var updated = false;
			if(packet.name==="OVERWRITE_DATA_TO_EXTERNAL_PAYTABLE"){
				var msgs = JSON.parse(data);
				for(var i=0;i<msgs.length;i++){
					var msg = msgs[i];
					if(msg.name==="OVERWRITE_DATA_TO_EXTERNAL_PAYTABLE"){
						updated=true;
						msg.id = packet.id;
						msg.payload = packet.payload;
						msg.origin = packet.origin;
						msg.timestamp = packet.timestamp;
					}
				}
				if(updated){
                    localStorage.setItem(INDEX_EMIT, JSON.stringify(msgs));
				}else{
    ...
    Intercom.prototype.readOutStorage = function() {
    	var self=this;
        this._transaction(function() {
            var now = intercomNow();//(new Date()).getTime();
            //load data from localStorage
	...
	Intercom.prototype.once = function(key, fn, ttl) {
		...
		this._transaction(function() {
			...
			data[key] = {};
			data[key].timestamp = intercomNow();//(new Date()).getTime();
